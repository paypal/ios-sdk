...
class PYPLCardContingencyRequest_Tests: XCTestCase {

    let cardContingencyRequest = PYPLCardContingencyRequest(contingencyURL: URL(string: "www.contingency.com")!)
    ...
    func testHandleRequest_callsOnPaymentWithURL_withProperContingencyURL() {
        ...
        mockPaymentFlowDriverDelegate.onPaymentWithURLVerifier = { url, error in
            XCTAssertEqual(url?.absoluteString, "www.contingency.com?redirect_uri=com.fake-return-url.scheme://x-callback-url/paypal-sdk/card-contingency")
            XCTAssertNil(error)
            expectation.fulfill()
        }
    ...
    func testCanHandleAppSwitchReturnURL_whenURLHasCorrectHostAndPath_returnsTrue() {
        XCTAssertTrue(cardContingencyRequest.canHandleAppSwitchReturn(URL(string: "com.braintreepayments.Demo.payments://x-callback-url/paypal-sdk/card-contingency")!, sourceApplication: ""))
    }
    
    func testCanHandleAppSwitchReturnURL_whenURLHasUnrecognizedHost_returnFalse() {
        XCTAssertFalse(cardContingencyRequest.canHandleAppSwitchReturn(URL(string: "com.braintreepayments.Demo.payments://some-other-host/paypal-sdk/card-contingency")!, sourceApplication: ""))
    }
    ...
    func testHandleOpenURL_whenSuccessOccurred_callsOnPaymentCompleteWithResult() {
        ...
        mockPaymentFlowDriverDelegate.onPaymentCompleteVerifier = { result, error in
            guard let result = result as? PYPLCardContingencyResult else { XCTFail(); return }
    ...
    func testPaymentFlowName() {
        XCTAssertEqual(cardContingencyRequest.paymentFlowName(), "paypal-sdk-contingency")
    }
}
